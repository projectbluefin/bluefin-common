...
| 4 | **Size budget immediately violated.** `docs/skills/lab-testing.md` is 906 lines; several others exceed the proposed 500-line hard limit. Phase E (per-skill directory migration) is intentionally deferred, so the plan as executed will be out of conformance with its own budget rule until Phase E. | ✅ resolved | Oversized skills are migrated to per-skill directories on sight — the size budget applies uniformly to every skill with no exemption list. |
