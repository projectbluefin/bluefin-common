...
| `lab-check.yml` | Receives `repository_dispatch` events of type `lab-check` from the ghost KubeVirt cluster and creates or updates a MergeRaptor-owned GitHub Check Run (`testing-lab / common`) for the PR SHA. Uses a short-lived MergeRaptor installation token scoped to `checks: write` only. Requires the org-wide MergeRaptor GitHub App to have `checks: write` in its installation permissions. | Changing how lab results are surfaced in the PR Checks UI |
